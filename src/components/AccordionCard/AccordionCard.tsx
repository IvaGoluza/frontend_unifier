...
const AccordionCard = ({ title, town, category, helpType, location, time, description, name, phoneNumber, email }: Data) => {
   ...
   return (
      <div className="request-container h-64 relative m-5 rounded-[18px] border bg-white pt-2 shadow-xl overflow-hidden md:max-w-xl">
         <div className="w-full grid grid-cols-4 pb-2 place-items-center border-b-2">
            <button className={`tablinks ${activeTab === 'Info' && 'active'} w-full text-center`} onClick={() => openTab('Info')}>
               <p className="border-r-2" data-title="Info">INFO</p>
            </button>
            <button className={`tablinks ${activeTab === 'Opis' && 'active'} w-full text-center`} onClick={() => openTab('Opis')}>
               <p className="border-r-2" data-title="Opis">OPIS</p>
            </button>
         ...
         <div id="Info" className={`tabcontent ${activeTab === 'Info' && 'active'} h-48 md:flex`}>
            ...
            <div className="grid grid-cols-1 grid-rows-4 lg:col-span-1 md:max-h-48 mt-3 ">
               ...
               <div className="infoDetails row-span-0.5 mt-2">
                  <FontAwesomeIcon icon={faLocationDot} style={{ marginRight: '5px' }} />{location}
               </div>
               <div className="infoDetails mb-5">
                  <FontAwesomeIcon icon={faClock} style={{ marginRight: '5px' }} />{time}
               </div>
            </div>
         </div>

         <div id="Opis" className={`tabcontent ${activeTab === 'Opis' && 'active'} w-full p-5 flex`}>
            <div>{description} </div>
         </div>
