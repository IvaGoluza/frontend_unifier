...
import { UserDetails } from "./UserProfileDetails";
...
const EditUserProfileModal: React.FC<EditUserProfileModalProps> = ({
  ...
  const getFieldComponent = (field: keyof UserDetails) => {
    const inputStyle =
      "mt-1 block w-full rounded-md border border-[#5422E1] shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm px-2";
    ...
    switch (field) {
      case "profileDescription":
        return (
          <div>
            ...
            <Field name="profileDescription" as="textarea" className={inputStyle} />
          </div>
        );
      case "workArea":
        return (
              ...
              {({ push, remove, form }) => (
                <div>
                  ...
                  <button
                    type="button"
                    onClick={() => push("")}
                    className="mt-2 rounded bg-[#5422E1] px-2 py-1 text-white hover:bg-[#4321a8]"
                  >
                    Dodaj područje rada
                  </button>
                </div>
      ...
      case "hasHealthCertificate":
        return (
          <div>
            <label className="block text-sm font-medium text-gray-700">Učitaj zdravstvenu potvrdu</label>
            <div className="mt-1 flex items-center">
              <input type="file" name="file" id="file-upload" className="hidden" onChange={handleFileChange} />
              <label
                htmlFor="file-upload"
                className="cursor-pointer rounded-[35px] bg-[#5422E1] px-2 py-1 text-white hover:bg-[#4321a8]"
              >
                Odaberi datoteku
              </label>
          ...
          {({ errors, touched }) => (
            <Form className="space-y-4">
              ...
              <div className="flex justify-end">
                <button
                  type="submit"
                  className="letter-spacing-0-06 font-krub flex h-[1.5rem] w-2/5 items-center justify-center rounded-[35px] bg-[#5422E1] p-2 text-xs font-bold text-white hover:bg-[#4321a8] lg:text-sm"
                >
                  Spremi
                </button>
              </div>
